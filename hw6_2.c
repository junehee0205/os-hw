...
int main(int argc, char *argv[])
{
	...
	for(int i = 0; i < NO_PHIL; i++){
		...
		param[i].state = &state[i];
		param[i].chopstick = &chopstick[i];
...
void* ThreadFn(void *vParam)
{
	...
	while(thread_cont){
		...
		while(1){
			if(state[(idx+no_phil-1)%no_phil] == 1 ||state[(idx+1)%no_phil] == 1);
			else break;
		}
		if((idx+2)%2 == 0){
			pthread_mutex_trylock(&chopstick[idx]);
			pthread_mutex_trylock(&chopstick[(idx+1)%no_phil]);
		}
		else if((idx+2)%2 !=0){
			pthread_mutex_trylock(&chopstick[(idx+1)%no_phil]);
			pthread_mutex_trylock(&chopstick[idx]);
